...
// 🔵 INFO: Define HTTP methods if not already defined
#ifndef HTTP_GET
typedef enum
{
  HTTP_GET = 0b00000001,
...
bool startAccessPoint()
{
  ...
  // 🔵 INFO: Configure and start access point
  // ⚪ NOTE: Provides local network for initial configuration
  WiFi.mode(WIFI_AP);
...
// ═══════════════════════════════════════════════════════════════════════════
// 🔵 INFO: WiFi Station Mode Connection
// ═══════════════════════════════════════════════════════════════════════════
...
bool connectToWiFi(const char *ssid, const char *password)
{
  ...
  // 🔵 INFO: Switch to station mode and attempt connection
  WiFi.mode(WIFI_STA);
...
// ═══════════════════════════════════════════════════════════════════════════
// 🔵 INFO: Helper Function - Convert EffectType to JSON Key
// ═══════════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════════
// 🔵 INFO: Helper Function - Create Effect JSON Object
// ═══════════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════════
// 🔵 INFO: Helper Function - Parse Effect from JSON
// ═══════════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════════
// 🔵 INFO: Web Server Route Handlers
// ═══════════════════════════════════════════════════════════════════════════

void setupRoutes()
{
  // ═══════════════════════════════════════════════════════════════════════
  // 🔵 INFO: Serve static files from LittleFS
  // ⚪ NOTE: HTML, CSS, JS files for web interface
  // ═══════════════════════════════════════════════════════════════════════

  server.serveStatic("/", LittleFS, "/").setDefaultFile("index.html");

  // ═══════════════════════════════════════════════════════════════════════
  // 🔵 INFO: GET /api/status - Current system status
  // ═══════════════════════════════════════════════════════════════════════
  ...
  server.on("/api/status", HTTP_GET, [](AsyncWebServerRequest *request)
    ...
    // 🔵 INFO: Sensor readings (if debug mode enabled)
  ...
  // ═══════════════════════════════════════════════════════════════════════
  // 🔵 INFO: GET /api/config - Get complete configuration
  // ═══════════════════════════════════════════════════════════════════════
  ...
  // ═══════════════════════════════════════════════════════════════════════
  // 🔵 INFO: POST /api/config - Update configuration
  // ═══════════════════════════════════════════════════════════════════════
  ...
  server.on("/api/config", HTTP_POST, [](AsyncWebServerRequest *request) {}, NULL, [](AsyncWebServerRequest *request, uint8_t *data, size_t len, size_t index, size_t total)
    ...
    // 🔵 INFO: All chunks received, now parse
      ...
      // 🔵 INFO: Update scheduling configuration
      ...
      // 🔵 INFO: Update sensor configuration and apply immediately
      ...
      // 🔵 INFO: Update global settings and apply immediately
  ...
  // ═══════════════════════════════════════════════════════════════════════
  // 🔵 INFO: POST /api/reset - Factory reset
  // ═══════════════════════════════════════════════════════════════════════
...
// 🟠 WARNING: Consider restarting ESP after factory reset
#ifdef DEBUG
  ...
  // ═══════════════════════════════════════════════════════════════════════
  // 🔵 INFO: POST /api/test - Manual effect trigger for testing
  // ═══════════════════════════════════════════════════════════════════════
  ...
  server.on("/api/test", HTTP_POST, [](AsyncWebServerRequest *request) {}, NULL, [](AsyncWebServerRequest *request, uint8_t *data, size_t len, size_t index, size_t total)
      ...
      // 🔵 INFO: Select configuration based on effect type
...
// ═══════════════════════════════════════════════════════════════════════════
// 🔵 INFO: OTA (Over-The-Air) Update Setup
// ═══════════════════════════════════════════════════════════════════════════

void setupOTA()
{
  // 🔵 INFO: Configure OTA hostname
  ArduinoOTA.setHostname(OTA_HOSTNAME);

  // 🔵 INFO: OTA event handlers for debugging
  ArduinoOTA.onStart([]()
                     {
    ...
    // 🔵 INFO: Stop all effects during update
    stopAllEffects(); });
...
// ═══════════════════════════════════════════════════════════════════════════
// 🔵 INFO: Web Server Initialization
// ═══════════════════════════════════════════════════════════════════════════
...
bool initWebServer(SystemConfig &config)
{
  ...
  {
    // 🔵 INFO: Start in AP mode for initial setup
    wifiSuccess = startAccessPoint();
  }
  ...
  // 🔵 INFO: Setup OTA updates if connected to home WiFi
...
// ═══════════════════════════════════════════════════════════════════════════
// 🔵 INFO: Web Server Update (Called in Main Loop)
// ═══════════════════════════════════════════════════════════════════════════

void updateWebServer()
{
  // 🔵 INFO: Handle OTA updates
  ArduinoOTA.handle();
